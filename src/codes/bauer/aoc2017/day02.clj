(ns codes.bauer.aoc2017.day02)

(def input [[3751	3769	2769	2039	2794	240	3579	1228	4291	220	324	3960	211	1346	237	1586]
...
(defn checksum1 [ss]
  (transduce (comp (map (juxt #(apply max %) #(apply min %)))
                   (map #(apply - %)))
             +
             ss))
...
(defn checksum2 [ss]
  (transduce (comp (map #(combinations % 2))
                   (map #(->> (map (comp reverse sort) %)
                              (some (fn [[n d]]
                                      (when (= 0 (mod n d))
                                        (/ n d)))))))
             +
